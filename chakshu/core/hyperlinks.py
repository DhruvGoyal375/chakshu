...
def extract_hyperlinks_from_paragraph(paragraph):
    ...
    for a_tag in soup.find_all('a', href=True):
        ...
        if href.startswith('/wiki/') and ':' not in href:  # Include only valid Wikipedia links
...
def main():
    ...
    if closest_links:
        for closest_link in closest_links:
            print(f"This is the closest link I found: {closest_link} -> {hyperlink_map[closest_link]}")
            response = input("Do you want to know about this link? (yes/no) ")
            
            if response.lower() == 'yes':
                print(f"Showing details for {closest_link}: {hyperlink_map[closest_link]}")
